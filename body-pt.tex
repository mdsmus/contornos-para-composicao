...
melodias populares \cite{adams76:melodic}. Na
...
artigo. Assim, descreveremos apenas as operações implementadas ou em
fase de implementação no \goiaba.
...
\begin{figure}
  \centering
  \subfigure[Cseg P]{
    \includegraphics[scale=.9]{cspace-5968}
    \label{fig:c-space5968}
  }
  \subfigure[Cseg M e csubseg O]{
    \includegraphics[scale=.9]{cspace-7420}
    \label{fig:c-space7420}
  }
  \subfigure[Cseg N]{
    \includegraphics[scale=.9]{cspace-564}
...
A operação de comparação \termo{COM} retorna a diferença de registro
...
normal, obtém-se \termo{CC} numerando-se ordenadamente todos os
\termo{c-pitch} de $0$ a $(n-1)$, sendo $n$ o número total de
...
contornos simples, considerando apenas os \termo{c-pitch}
\verb!(5 9 6)!, e como contornos com duração, considerando também o
local no tempo onde ocorre cada \termo{c-pitch}:
\verb!((0 5)(1 9)(2 6)!. Apesar de termos uma codificação específica
que comporte a influência do tempo no contorno, ainda não estamos trabalhando com
esse elemento.
...
\begin{figure}
  \centering
  \includegraphics[scale=.4]{contornos}
