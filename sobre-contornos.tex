...
notas, podem ajudar com a coerência musical de uma obra. 

\citacaolonga{Continuity and coherence are organizational principles
  that operate at various levels, from the smallest formal unit to the
  entire composition. To be artistically effective, these principles
  cannot be limited to simple repetition but must exist in contexts
  where musical events are differentiated by one means or another. The
  introduction of contrasting events, interruptive devices, and
  cadential punctuations are important means for differentiating,
  highlighting, and separating musical events in time} {Continuidade e
  coerência são princípios organizacionais que operam em vários
  níveis, da menor unidade formal até a composição como um todo. Para
  ser artisticamente efetivo, estes princípios não podem ser limitados
  à simples repetição, mas devem existir em contextos onde eventos
  musicais são diferenciados por um meio ou por outro. A introdução de
  eventos contrastantes, dispositivos de interrupção, e pontuações
  cadenciais são meios importantes para a diferenciação, destaque e
  separação de eventos musicais no tempo}{p. 296}{kliewer75:aspects}
